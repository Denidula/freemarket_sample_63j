...
$(document).on('turbolinks:load', function () {
  ...
  var this_year = date.getFullYear();
  var oldest_year = 1900;
  var limit_year = this_year + 10;

  // 誕生年月の配列を自動生成
  for (var i = this_year; i <= limit_year; i++) {
    var year = { var: i, txt: String(i) };
    birth_year.push(year);
  }
  for (var i = 1; i <= 12; i++) {
  ...
  for (var i = 0; i < birth_year.length; i++) {
    $("<option>", {
      value: birth_year[i].var,
      text: birth_year[i].txt
    }).appendTo('#select_limit_year');
  }
  for (var i = 0; i < birth_month.length; i++) {
    $("<option>", {
      value: birth_month[i].var,
      text: birth_month[i].txt
    }).appendTo('#select_limit_month');
  }

  // 年、もしくは月のセレクトボックスの中身に変更があったら日の内容を変更する
  $('#select_limit_year, #select_limit_month').change(function () {
    // 日が入っているセレクトボックスの中身を空っぽにする
    $('#select_limit_day').empty();
    // 日が入るセレクトボックスにデフォルト値の'--'を入れる
    $("<option>", {
      text: '--'
    }).appendTo('#select_limit_day');
    // ２月の日にち、すなわちmonths[1]を28にセットし直す
    months[1] = 28;
    //　年と月に入力されている情報を取得する
    var year = $('#select_limit_year').val();
    var month = $("#select_limit_month").val();
    // yearとmonth共に数字が入力されている、すなわち共に'--'でないときに実行
    if (year != '--' && month != '--') {
      // monthに２月が選択されている時実行
      if (month == 2) {
